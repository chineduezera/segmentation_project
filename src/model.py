...
class Unet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.down1 = nn.Sequential(
            self.maxpool, RepeatedConv(in_channels=64, out_channels=128)
        )
        self.down2 = nn.Sequential(
            self.maxpool, RepeatedConv(in_channels=128, out_channels=256)
        )
        self.down3 = nn.Sequential(
            self.maxpool, RepeatedConv(in_channels=256, out_channels=512)
        )

        # Bottleneck layer
        self.down4 = nn.Sequential(
            self.maxpool, RepeatedConv(in_channels=512, out_channels=1024)
        )
